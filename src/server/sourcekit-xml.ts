...
interface Root {
    elements: [NamedElement<'Function' | 'Class', Array<Name | Declaration | USR | CommentParts>>]
}
...
type Block = NamedElement<'Abstract' | 'Discussion' | 'Item' | 'ResultDiscussion' | 'ThrowsDiscussion', Array<Para | CodeListing>>;
type CodeListing = NamedElement<'CodeListing', Array<CodeLineNumbered>, { language?: string }>;
...
function skElement2md(element: SkElement): string {
    const children = (opt: { sep?: string, map?: (v: string, i: number) => string } = {}, els?: SkElement[]) => (els || element.elements as Array<SkElement> || [])
        .map(sk2md)
    ...
    switch (element.name) {
        ...
        case SkElementType.Direction: // unknown
            return ''
        default:
            return '**<' + element.name + '>** ' + children() + '**<' + element.name + '>** ';
    }
}
